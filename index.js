...
// Check and notify tasks
const checkAndNotifyTasks = async (socket) => {
  try {
      ...
      for (const key in tasks) {
        if (tasks.hasOwnProperty(key)) {
          ...
          if (remainingDays === 1) {
            ...
            const message = `T.G.D ${task.dosen}, ${task.namaTugas}`;
            await query({ question: message }, sessionId);
            console.log(`Notifikasi 1 hari tersisa untuk ${task.namaTugas}`);
            logToFile(`Notifikasi 1 hari tersisa untuk ${task.namaTugas}`);
          } else if (remainingDays === 0) {
            await deleteData(`tugas/${key}`);
            console.log(`Tugas ${task.namaTugas} dihapus karena deadline 0 hari`);
            logToFile(`Tugas ${task.namaTugas} dihapus karena deadline 0 hari`);
          }
        }
      }
    }
  } catch (error) {
    console.error('Error saat memeriksa tugas:', error);
    logToFile('Error saat memeriksa tugas: ' + error.message);
  }
};
  ...
  try {
    ...
    socket.ev.on("connection.update", ({ connection, qr }) => {
      if (connection === 'open') {
        ...
        // Mulai pemantauan tugas
        setInterval(() => checkAndNotifyTasks(socket), 24 * 60 * 60 * 1000); // Periksa setiap hari
